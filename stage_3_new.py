...
# Return the mean of the largest 5 pairs
def match(probe, gallery):
    return np.mean(np.sort(cosine_similarity(probe, gallery).flatten())[-5:])
...
if __name__ == '__main__':
    ...
    np.savetxt('../Shared/AS/stage_3/new_raw.txt', sorted(result, key=lambda p: p[2], reverse=True)[:10000], delimiter=' ', fmt='%s')
